...
public class RoomManager
{
    ...
    private void HandleCreateRoom(NetClientHandler handler, NetworkMessage message)
    {
        try
        {
            ...
            {
                SendResponseToClient(handler, "CREATE_ROOM", false, "Invalid room creation request");
                return;
            }
            ...
            {
                SendResponseToClient(handler, "CREATE_ROOM", false, "Failed to create room");
            }
        }
        catch (Exception ex)
        {
            SendResponseToClient(handler, "CREATE_ROOM", false, $"Error creating room: {ex.Message}");
        }
    }
    ...
    private void HandleJoinRoom(NetClientHandler handler, NetworkMessage message)
    {
        try
        {
            ...
            {
                SendResponseToClient(handler, "JOIN_ROOM", false, "Room name not specified");
                return;
            }

            if (JoinRoom(message.RoomName, message.Username))
            {
                SendResponseToClient(handler, "JOIN_ROOM", true, "Joined room successfully");
                _ = BroadcastRoomListAsync();
            }
            else
            {
                SendResponseToClient(handler, "JOIN_ROOM", false, "Failed to join room");
            }
        }
        catch (Exception ex)
        {
            SendResponseToClient(handler, "JOIN_ROOM", false, $"Error joining room: {ex.Message}");
        }
    }
    ...
    private void HandleWatchRoom(NetClientHandler handler, NetworkMessage message)
    {
        try
        {
            if (string.IsNullOrEmpty(message.RoomName))
            {
                SendResponseToClient(handler, "WATCH_ROOM", false, "Room name not specified");
                return;
            }

            if (WatchRoom(message.RoomName, message.Username))
            {
                SendResponseToClient(handler, "WATCH_ROOM", true, "Started watching room");
            }
            else
            {
                SendResponseToClient(handler, "WATCH_ROOM", false, "Failed to watch room");
            }
        }
        catch (Exception ex)
        {
            SendResponseToClient(handler, "WATCH_ROOM", false, $"Error watching room: {ex.Message}");
        }
    }
    ...
    private void SendResponseToClient(NetClientHandler handler, string action, bool success, string message)
    {
        var response = new NetworkMessage
        {
            Action = action,
